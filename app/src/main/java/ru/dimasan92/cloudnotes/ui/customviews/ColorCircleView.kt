...
class ColorCircleView @JvmOverloads constructor(
    ...
    @Dimension(unit = PX) var radius = dip(defRadiusDp).toFloat()
    @ColorRes var fillColorRes = R.color.color_white
        set(value) {
            field = value
            fillPaint.color = ContextCompat.getColor(context, value)
        }
    @ColorRes var strokeColorRes = R.color.color_text_secondary
        set(value) {
            field = value
            strokePaint.color = ContextCompat.getColor(context, value)
        }
    @Dimension(unit = PX) var strokeWidth = dip(defStrokeWidthDp).toFloat()
        set(value) {
            field = value
    ...
    init {
        ...
        strokeColorRes =
            a.getResourceId(R.styleable.ColorCircleView_strokeColor, R.color.color_text_secondary)
        a.recycle()
    }
